...
void main() {
  group('AuthCubit', () {
    ...
    final decodedToken = JwtDecoder.decode(validToken);

    setUp(() {
      mockJwtOps = MockJwtOps();
      mockOneSignal = MockOneSignal();
      when(mockJwtOps.decodeJWT(validToken)).thenReturn(decodedToken);
    });
    group('appStarted()', () {
      ...
      blocTest<AuthCubit, AuthState>(
        ...
        build: () {
          ...
          return AuthCubit(mockJwtOps, mockOneSignal);
        },
        act: (cubit) => cubit.appStarted(),
    ...
    group('loggedIn()', () {
      blocTest<AuthCubit, AuthState>(
        ...
        build: () => AuthCubit(mockJwtOps, mockOneSignal),
        act: (cubit) => cubit.loggedIn(validToken),
        expect: [AuthState.authenticated()],
        verify: (_) {
          verify(mockJwtOps.setDefaultAuthHeader(validToken));
          verify(mockOneSignal.sendTags({'user_id': decodedToken['id']}));
        },
      );
    });
    ...
    group('loggedOut()', () {
      blocTest<AuthCubit, AuthState>(
        ...
        expect: [AuthState.unauthenticated()],
        verify: (_) {
          verify(mockOneSignal.deleteTag('user_id'));
        },
      );
    });
  });
